...
function getWeekday($yeart, $montht, $dayt) {
	$todayt=jalali_to_gregorian($yeart, $montht, $dayt) ;
	$date =$todayt[0]."-".$todayt[1]."-".$todayt[2]; 
    return jdate('l', strtotime($date));
}



function miladi_to_jalali ($miladitime,$sep="/",$mid="T") 

{
 $g_y=substr($miladitime,0,4);
 	$g_m=substr($miladitime,5,2);
	$g_d=substr($miladitime,8,2);
	$g_t=substr($miladitime,11,12);
   ...
   if($jd<10)
	...
	$jalalidate=$jy.$sep.$jm.$sep.$jd.$mid.$g_t;
   return $jalalidate; 
...
function jalali_to_miladi($jalalitime,$sep="/",$mid="T") 

{ 
$j_y=substr($jalalitime,0,4);
 	$j_m=substr($jalalitime,5,2);
	$j_d=substr($jalalitime,8,2);
	$j_t=substr($jalalitime,11,12);
...
$miladidate=$gy.$sep.$gm.$sep.$gd.$mid.$j_t;
   return $miladidate; 
...
function difference_date($startdate, $enddate) {
	$exs=explode('/',$startdate);
	$exe=explode('/',$enddate);
	
	if($exs[1]<10) $exs[1]='0'.$exs[1];
	if($exs[2]<10) $exs[2]='0'.$exs[2];
	
	$sd=jmaketime(0,0,0,$exs[1],$exs[2],$exs[0]);
	$ed=jmaketime(0,0,0,$exe[1],$exe[2],$exe[0]);
