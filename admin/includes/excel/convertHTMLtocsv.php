...
class convertHTMLtoscv{
	...
	*/
	public function generateExcel($content,$configExcel){
		...
		return( TMP_FILES . $this->fileName . '.csv' );
	}
	...
	*/
	public function excel($configExcel){
	   ...
	   /* simple style to make sure all cell's text have HORIZONTAL_LEFT alignment */
		...
		if($configExcel['mainTitleCell'])
		foreach( $configExcel['mainTitleCell'] as $maintitle ){
			$objPHPExcel->getActiveSheet()->getStyle($maintitle)->applyFromArray($mainTitleStyle);		
		}
		if($configExcel['secoundTitleCell'])
		foreach( $configExcel['secoundTitleCell'] as $secoundtitle ){
			$objPHPExcel->getActiveSheet()->getStyle($secoundtitle)->applyFromArray($secoundTitleStyle);		
		}
		...
		$excelFile = TMP_FILES . $this->fileName . '.csv'; // create excel file under temp folder.
...
//		$objPHPExcel->getActiveSheet()->getStyle('A1:Z3')->getAlignment()->setWrapText(true); 
	   ...
	   /* simple style to make sure all cell's text have HORIZONTAL_LEFT alignment */
		...
		//Apply the style
	        $objPHPExcel->getActiveSheet()->getDefaultStyle()->applyFromArray($style);
    		$objPHPExcel->getActiveSheet()->getStyle("A1:U4")->applyFromArray($headerStyle);	
	        $excelFile = $filename . '.csv'; // create excel file under temp folder.
...
//		$objPHPExcel->getActiveSheet()->getStyle('A1:Z3')->getAlignment()->setWrapText(true); 
	   ...
	   /* simple style to make sure all cell's text have HORIZONTAL_LEFT alignment */
		...
		//Apply the style
	        $objPHPExcel->getActiveSheet()->getDefaultStyle()->applyFromArray($style);
    		$objPHPExcel->getActiveSheet()->getStyle("A1:U4")->applyFromArray($headerStyle);	
	        $excelFile = TMP_FILES . $filename . '.csv'; // create excel file under temp folder.
...
//		var_dump($objPHPExcel);
	...
	public function downloadFile() {
		...
		if(file_exists($fileName) && ($fileType == 'html' || $fileType == 'csv')) {
			if($fileType == 'csv') {
				header('Content-type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
				header('Pragma: ');
				header('Cache-Control: ');
