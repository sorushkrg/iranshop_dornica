...
if (!function_exists("_ktx")) {
    function _ktx($data, $param = null)
    {
        if (empty_null($data)) return $data;
        return KT_escapeAttribute($data, $param);
    }
}

if (!function_exists('_replace_')) {
    function _replace_($matche)
    {
        if ($matche[0] == "\'")
            return  "'";
        elseif ($matche[0] == '\\')
...
if (!function_exists("remove_php_scape")) {
    function remove_php_scape($value)
    {
        ...
        switch ($valueType) {
            case 'string':
                return preg_replace_callback($pattern, '_replace_', $value);
                break;
            case 'array':
                foreach ($value as $key => $val)
                    $newValue[$key] = preg_replace_callback($pattern, '_replace_', $val);
                return $newValue;
                break;
            case 'object':
                foreach ($value as $key => $val)
...
if (!function_exists("GetSQLValueString")) {
    ...
    {
        $theValue = remove_php_scape($theValue);

        $theValue = str_replace("ي", "ی", trim($theValue));
        $theValue = str_replace("ئ", "ی", trim($theValue));
        $theValue = str_replace("ي", "ی", trim($theValue));
        $theValue = str_replace("ى", "ی", trim($theValue));
        $theValue = str_replace('ك', 'ک', trim($theValue));
...
function select_triger($val1, $val2, $val3)
{
    global $cn;
    $dltr = _ktx($val1, ["dltr", 'de']);
    $query_rs1 = "SELECT * FROM " . trim($dltr) . ' WHERE ' . $val2 . ' = ' . $val3;

    $rs1 = mysqli_query($cn, $query_rs1) or die(sql_error_handler($cn));
    $row_rs1 = mysqli_fetch_assoc($rs1);
    $fieldcount = mysqli_num_fields($rs1);
    for ($kk = 0; $kk < $fieldcount; $kk++) {
        $colNamekk = mysqli_fetch_field_direct($rs1, $kk);
        $name = (string) KT_escapeForSql($row_rs1[$colNamekk->name], '');
        $tmfinallist[] = KT_escapeFieldName($colNamekk->name) . ' : ' . trim($name, "'");
    }

    $ret = implode(" , ", $tmfinallist);
...
function KT_transformsPaths($templateUrl, $text, $makeAbsolut)
{
    ...
    foreach ($matches[2] as $key => $value) {
        ...
        if (isset($mmatch[2])) {
            ...
            if (
                !($mmatch[1] == 'href' && strpos($mmatch[2], 'javascript:') === 0)
...
/**
 * Return ips of the localhost which includes of ipv4 and ipv6
 * @return array return the localhost ips (ex. [127.0.0.1]);
 */
...
/**
 * Return the real ip of the user in new server architecture.
 * @return string return the user ip (ex. 1.1.1.1 );
 */
 ...
 */
function KT_escapeAttribute($inputval, $params = null)
{
    ...
    if (is_array($values)) {
        foreach ($values as $key => $val) {
            ...
            $val = str_replace("ي", "ی", trim($val));
            $val = str_replace("ئ", "ی", trim($val));
            $val = str_replace("ي", "ی", trim($val));
            $val = str_replace("ى", "ی", trim($val));
            $val = str_replace('ك', 'ک', trim($val));
            $val = str_replace("javascript", "", $val);

            $val = remove_php_scape($val);
            $res = '';
            if (is_null($params) || empty($params)) {
                $val = strip_tags($val);
                // $val = filter_var($val, FILTER_SANITIZE_STRING); session filter list khrab mikone 
                $res = str_replace(array('"', "<", ">"), array("&quot;", "&lt;", "&gt;"), $val);
                ...
                if (strpos($val, ',') !== false || gettype($val) === 'array') {
                    $data = (strpos($val, ',') !== false) ? explode(',', $val) : $val;
                    foreach ($data as $value) {
                        if ($params[1] == 'de')
                            $dres[] = decrypt($value, session_id() . $params[0]);
                        else
                            $dres[] = encrypt($value, session_id() . $params[0]);
                    }
                    $res = implode(',', $dres);
                } else {
                    if ($params[1] == 'de')
                        $res = decrypt($val, session_id() . $params[0]);
                    else
                        $res = encrypt($val, session_id() . $params[0]);
                }
            } else $res = $val;
            $resarr[$key] = $res;
        }
...
function KT_escapeAttributedb($val, $params = null)
{
    $val = str_replace("ي", "ی", trim($val));
    $val = str_replace("ئ", "ی", trim($val));
    $val = str_replace("ي", "ی", trim($val));
    ...
    $res = '';
    // if($params != 'ckeditor' and $params != 'CKEDITOR'){
    if (is_null($params) || empty($params)) {
        $val = strip_tags($val);
...
function KT_setDbType($connection)
{
    ...
    if (!isset($GLOBALS['KT_serverModel'])) {
        ...
        if (isset($connection->servermodel) && $connection->servermodel == 'mysqli') {
            $GLOBALS['KT_serverModel'] = 'mysqli';
        }
    }
...
function KT_escapeFieldName($colName)
{
    ...
    if (isset($GLOBALS['KT_dataDbType'])) {
        if (strpos($GLOBALS['KT_dataDbType'], 'mysqli') !== FALSE) {
            $startq = '`';
            $endq = '`';
        }
