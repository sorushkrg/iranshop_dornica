...
function MXW_CommaCheckboxes (input) {
	...
	this.displaycat = WDG_getAttributeNS(this.input, 'displaycat');
	this.displaycatname = WDG_getAttributeNS(this.input, 'displaycatname');
	this.descriptionfield = WDG_getAttributeNS(this.input, 'descriptionfield');
...
function MXW_CommaCheckboxes_paint(forceRepaint) {
	...
	// NEW VERSION
	var theDiv = utility.dom.createElement("DIV", {
		class:'8'
	});
	
	theDiv.setAttribute("style", "background-color:#FFFFFF;");
	// NEW VERSION
	...
	var oldcat=this.recordset.Fields(this.displaycat);
	while(this.recordset.MoveNext()) {
		if(oldcat!=this.recordset.Fields(this.displaycat) )
		{
		cellCount = 0;
		curRow = null;	
		oldcat=this.recordset.Fields(this.displaycat);
		
		curRow = theTable.insertRow( (is.safari ? -1 : theTable.rows.length) );
			curCell = curRow.insertCell(curRow.cells.length);
			curCell.colSpan=4;
			curCell.bgColor="#f8f8f8";
			curCell.height="30";
			curCell.innerHTML=this.recordset.Fields(this.displaycatname);
	
			// NEW VERSION

			var TDiv = utility.dom.createElement("DIV", {
				class:'col-md-12'
			});
			
			TDiv.innerHTML = this.recordset.Fields(this.displaycatname);
			TDiv.style.height = "30px";
			TDiv.setAttribute("style", "vertical-align:middle;padding:12px 20px;line-height:1.5384616;border-top:1px solid #ddd;background-color:#f8f8f8;"); //Set div attributes
		...
		lbl = utility.dom.createElement("LABEL", {
			...
			"title": this.recordset.Fields(this.descriptionfield),
			"data-popup":"tooltip",
		...
		var EDiv = utility.dom.createElement("DIV", {
			class:'col-md-3'
		});
		
        EDiv.setAttribute("style", "vertical-align:middle;padding:12px 20px;line-height:1.5384616;border-top:1px solid #ddd;background-color:#FFFFFF;height:60px;"); //Set div attributes
		
		theDiv.appendChild(EDiv);
	...
	// NEW VERSION
	this.container.appendChild(theDiv);	
	// NEW VERSION
...
function MXW_CommaCheckboxes_inspect() {
    ...
    // START - perloader part
    setTimeout(function (){
        $(".mask_loading").hide();
    }, ((this.checkboxes.length + 1) * 10) + 500)
    // END - preloader part
