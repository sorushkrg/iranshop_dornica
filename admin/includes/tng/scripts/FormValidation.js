		...
		if (typeof spn == 'undefined') {
			...
			if (el.type) {
				if (el.type.toString().toLowerCase() == 'radio') {
					try {
						spn = el.closest(".radio-inline");
					} catch(e) { spn = el; }
				}else if (el.type.toString().toLowerCase() == 'file') {
					if(el.closest("div.uploader") != null){
						spn = el.closest("div.uploader");
					}else{
						spn = el;
					}
				}
...
function UNI_form_attach() {
	...
	if (div.length == 1) {
		...
		start_el = utility.dom.getElementsByClassName(document, 'KT_tngtable', 'div')[0];
	}
	...
	Array_each(utility.dom.getElementsByClassName(document, 'KT_tngtable', 'div'), function(table, table_index){
