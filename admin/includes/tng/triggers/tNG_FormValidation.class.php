...
class tNG_FormValidation {
	...
	function loadValidatioRules(&$arr) {
		$arr['text'] = array();
		$arr['text'][''] = array();
		$arr['text']['pss']['regexp'] = '/^.*([a-zA-Z].*[0-9]|[0-9].*[a-zA-Z]).*$/';
		$arr['text']['email']['regexp'] = '/^\w+[\w\+\.\-]*@\w+(?:[\.\-]\w+)*\.\w+$/i';
		$arr['text']['mobile']['regexp'] = '/^09(1[0-9]|3[0-9]|2[0-9]|0[1-9]|9[0-9])-?[0-9]{3}-?[0-9]{4}$/';
		$arr['text']['zipcode']['regexp'] = '/^\d{10}$/';
		$arr['text']['codemeli']['regexp'] = '/^\d{10}$/';
