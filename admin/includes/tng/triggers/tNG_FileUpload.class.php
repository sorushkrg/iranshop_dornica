...
class tNG_FileUpload{
	...
	function Execute() {
		...
		if ($this->dbFieldName != '') {
			...
			$arrex=explode(".",$saveFileName);			
			if(sizeof($arrex)>2){
				$errObj =  new tNG_error("FILE_EXTENTION_ERROR", array(),array($this->dbFieldName));
				return $errObj;
			}
		...
		$fileUpload->setUploadType($this->uploadType);
		...
		$this->uploadedFileName = $fileUpload->uploadFile($saveFileName, $oldFileName,$this->uploadType);
