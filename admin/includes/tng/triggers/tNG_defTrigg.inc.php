...
function Trigger_Login_AutoLogin(&$tNG) {
	...
	if (isset($tNG->columns['kt_login_rememberme']) && $tNG->getColumnValue('kt_login_rememberme')!="") { 
		...
		$expires = time() + intval($GLOBALS['tNG_login_config']['autologin_expires']) * 24 * 60 * 60;
		$coockiData = [
			'expires' => $expires, 
			'path' => $cookie_path,
			'domain' => '', 
		...
		setcookie("kt_login_id", $tNG->getColumnValue('kt_login_id'), $coockiData);

		$kt_test = tNG_encryptString($tNG->getColumnValue('kt_login_password_db'));
		setcookie("kt_login_test", $kt_test,  $coockiData);
	}
	return null;
}
