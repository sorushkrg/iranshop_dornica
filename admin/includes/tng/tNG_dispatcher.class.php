...
class tNG_dispatcher
{
    ...
    {
        $formvalidation_js_version = filemtime($this->relPath . "../../includes/tng/scripts/FormValidation.js");
        $outRules = '';
        $outRules .= '<script src="' . $this->relPath . '../../includes/tng/scripts/FormValidation.js?v=' . $formvalidation_js_version . '" type="text/javascript"></script>' . "\r\n";
    ...
    function getErrorMsg()
    {
        ...
        if ($errorWasFound) {
            ...
            if ($ret_user != '') {
                $ret .= '<div id="KT_tngerror">
										<label>' . KT_getResource('ERROR_LABEL', 'tNG') . '</label><div>' . $ret_user . '</div>
									</div>' . "\r\n";
