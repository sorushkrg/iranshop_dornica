...
class tNG
{
	...
	{
		new tNG_log('tNG' . $this->transactionType, 'executeTransaction', 'begin');
		if ($this->started) {
			new tNG_log('tNG' . $this->transactionType, 'executeTransaction', 'end');
			return false;
		}
		...
		if (isset($this->triggers[$triggerType]) && is_array($this->triggers[$triggerType])) {
			...
			foreach ($this->triggers[$triggerType] as $key => $trigger) {
				...
				if ($runTrigger) {
					...
					new tNG_log($triggerType, $callBackFunction, 'begin');
					...
					if ($triggerType != "ERROR") {
						if (is_object($ret)) {
							...
							new tNG_log($triggerType, $callBackFunction, 'end');
							return $ret;
						}
					}
					new tNG_log($triggerType, $callBackFunction, 'end');
	...
	{
		new tNG_log('tNG' . $this->transactionType, 'doTransaction', 'begin');

		$tmp = $this->getError();
		if (is_object($tmp)) {
			$this->setError($tmp);
			$this->executeTriggers("ERROR");
			new tNG_log('tNG' . $this->transactionType, 'doTransaction', 'end');
			return false;
		}
		...
		if ($this->sql != '') {
			new tNG_log('tNG' . $this->transactionType, 'executeTransaction', 'execute sql');
