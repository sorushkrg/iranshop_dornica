...
class tNG_insert extends tNG_fields
{
    ...
    function prepareSQL()
    {
        new tNG_log('tNG_insert', 'prepareSQL', 'begin');
        parent::prepareSQL();
        ...
        foreach ($this->columns as $colName => $colDetail) {
            ...
            $colTetype = $colDetail['tetype'];
            $colValue = $colDetail['value'];
            $colMethod = $colDetail['method'];

            $colValue = str_replace("ي", "ی", trim($colValue));
            $colValue = str_replace("ئ", "ی", trim($colValue));
            $colValue = str_replace("ي", "ی", trim($colValue));
            $colValue = str_replace("ى", "ی", trim($colValue));
            $colValue = str_replace('ك', 'ک', trim($colValue));
            ...
            if ($colMethod != 'HIDDEN') {
                ...
                if ($colType == "FILE_TYPE") {
                    ...
                    $tmValStr = $tmValStr . $sep . KT_escapeForSql(KT_escapeAttributedb($colValue, $colTetype), $colType);
                }
                $tmfinallist[] = KT_escapeFieldName($colName) . ' : ' . trim((string)KT_escapeForSql(KT_escapeAttributedb($colValue, $colTetype), $colType), "'");
            }
        }
        if (!$KT_sp) {
        ...
        // @session_start();
        // $logtype = 1;
        // $loguser = $_SESSION['auser'];
        // $logdate = jdate("YmdHis");
        // $logid = $id;
        // $logtext = implode(" , ", $tmfinallist);
        // $logip = get_real_ip_addr();
        // $logtable = $this->table;
        // $logtitle = $_SESSION['cur_pagename'];
        // $combined = [
        //     $logtype, $loguser, $logdate, $logip, $logtable, $logtitle, $logid, $logtext
        // ];
        // $_SESSION['DataToBeLogged'] = $combined;

       new tNG_log('tNG_insert', 'prepareSQL', 'end');
        return null;
    }
    ...
    {
        new tNG_log('tNG_insert', 'getLocalRecordset');
        $fakeArr = array();
        $tmpArr = $this->columns;
     ...
     * @access public
     */
    function addColumn($colName, $type, $method, $reference, $defaultValue = '', $tetype = '')
    {
        parent::addColumn($colName, $type, $method, $reference, $tetype);

        $reference = str_replace("ي", "ی", $reference);
        $reference = str_replace("ى", "ی", $reference);
        $reference = str_replace("ئ", "ی", $reference);
        $reference = str_replace('ك', 'ک', $reference);

        if ($method == "VALUE") {
            $this->columns[$colName]['tetype'] = $tetype;
    ...
    {
        new tNG_log('tNG_insert', 'postExecuteSql');
        ...
        $_SESSION['DataToBeLogged'][6] = $this->primaryKeyColumn['value'];

        // $dataToBeLogged = $_SESSION['DataToBeLogged'];
        // insert_triger($dataToBeLogged[0], $dataToBeLogged[1], $dataToBeLogged[2], $dataToBeLogged[3], $dataToBeLogged[4], $dataToBeLogged[5], $dataToBeLogged[6], $dataToBeLogged[7]);
        unset($_SESSION['DataToBeLogged']);
        return null;
    }
