...
class TFI_TableFilter
{
    ...
    public function __construct($connection, $filterName)
    {
        KT_setDbType($connection);
        $this->filterName = _ktx($filterName);

        KT_SessionKtBack(KT_getFullUri());
        $_SESSION['KT_lastUsedList'] = substr($filterName, 4);
    }

    function addColumn($colName, $type, $reference, $compareType, $default = '')
    {

        $reference = _ktx($reference);
        ...
        if (gettype($default) == 'array') {
            $this->columns[$colName][$reference]['tetype'] = $default;
            $defaultValue = '';
        } else $defaultValue = _ktx($default);
        ...
        if (!isset($_SESSION[$this->filterName . "_" . $reference])) {
            $_SESSION[$this->filterName . "_" . $reference] = _ktx($defaultValue);
        }
    }
    ...
    function Execute()
    {
            ...
            foreach ($this->columns as $columnName => $columnDetails) {
                foreach ($columnDetails as $key => $details) {
                    ...
                    if (isset($_POST[$variableName])) {
                        ...
                        if (isset($details['tetype'])) {
                            $details['tetype'][1] = isset($details['tetype'][1]) ? $details['tetype'][1] : 'de';
                            $_SESSION[$variableName] = _ktx($details['value'], $details['tetype']);
                        } else
                            $_SESSION[$variableName] = _ktx($details['value']);
                    } else {
        ...
        foreach ($this->columns as $columnName => $columnDetails) {
            foreach ($columnDetails as $key => $details) {
                ...
                $variableValue = _ktx(trim($details['value']));

                /*** تبدیل حروف عربی به فارسی*/
                $variableValue = str_replace("ي", "ی", $variableValue);
                $variableValue = str_replace('ك', 'ک', $variableValue);
                /*** */
                ...
                switch ($details['type']) {
                    ...
                    default:
                        ...
                        $condition .= ($compareType != 'MATCH' ? KT_escapeFieldName($columnName) . ' ' . $compareType . ' ' . $variableValue : "MATCH " . KT_escapeFieldName($columnName) . " AGAINST($variableValue IN BOOLEAN MODE)");
                        break;
                }
            }
        }
